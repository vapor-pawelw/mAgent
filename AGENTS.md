...
- **Local module split contract**: Shared non-UI code lives in `Packages/MagentModules`. Keep resource-heavy AppKit UI, `ThreadManager`, banners, and update/IPC orchestration in the app target unless you first introduce package-safe resource/theme/localization wrappers. Prefer extracting pure models/services/utilities into package targets rather than moving AppKit controllers directly.
