...
final class AgentSetupService {
    ...
    func ensureManagedZdotdir() -> String {
        ...
        for fileName in ManagedShellStartup.managedFiles {
            ...
            let desired = ManagedShellStartup.contents(for: fileName)
    ...
    func agentStartCommand(
        ...
        return "\(envExports) && exec env MAGENT_START_CWD=\(startCwd) \(ManagedShellStartup.profileVariable)=\(ManagedShellStartup.agentProfile) ZDOTDIR=\(zdotdir) \(shell) -il -c \(ShellExecutor.shellQuote(innerCmd))"
    }
