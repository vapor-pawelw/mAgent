...
extension ThreadManager {
    ...
    func cleanupStaleMagentSessions() async -> [String] {
        ...
        let staleSessions = liveSessions.filter { sessionName in
            TmuxSessionNaming.isMagentSession(sessionName) && !referencedSessions.contains(sessionName)
        }
    ...
    private func referencedMagentSessionNames() -> Set<String> {
        ...
        for thread in allNonArchivedThreads {
            for sessionName in thread.tmuxSessionNames where TmuxSessionNaming.isMagentSession(sessionName) {
                names.insert(sessionName)
            }
            for sessionName in thread.agentTmuxSessions where TmuxSessionNaming.isMagentSession(sessionName) {
                names.insert(sessionName)
            }
            for sessionName in thread.pinnedTmuxSessions where TmuxSessionNaming.isMagentSession(sessionName) {
                names.insert(sessionName)
            }
            if let selectedSession = thread.lastSelectedTmuxSessionName, TmuxSessionNaming.isMagentSession(selectedSession) {
