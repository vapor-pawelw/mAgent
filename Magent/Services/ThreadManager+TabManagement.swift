...
extension ThreadManager {
    ...
    func addTab(
        ...
        let repoSlug = TmuxSessionNaming.repoSlug(from:
        ...
        if currentThread.isMain {
            ...
            let tabSlug = TmuxSessionNaming.sanitizeForTmux(tabDisplayName)
            let baseName = TmuxSessionNaming.buildSessionName(repoSlug: repoSlug, threadName: nil, tabSlug: tabSlug)
            ...
            let tabSlug = TmuxSessionNaming.sanitizeForTmux(tabDisplayName)
            let baseName = TmuxSessionNaming.buildSessionName(repoSlug: repoSlug, threadName: currentThread.name, tabSlug: tabSlug)
            var candidate = baseName
            var suffix = 2
            while await isTabNameTaken(candidate, existingNames: existingNames) {
                candidate = "\(baseName)-\(suffix)"
    ...
    func baseTabDisplayName(for agentType: AgentType?) -> String {
        switch agentType {
        case .claude:
            return "Claude"
        case .codex:
