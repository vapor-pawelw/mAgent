...
final class StatusBarView: NSView, NSPopoverDelegate {
    ...
    private func setupViews() {
        ...
        trailingThreadStatusStack.orientation = .horizontal
        trailingThreadStatusStack.alignment = .centerY
        trailingThreadStatusStack.spacing = 8
        trailingThreadStatusStack.translatesAutoresizingMaskIntoConstraints = false
        trailingThreadStatusStack.setContentHuggingPriority(.required, for: .horizontal)
        trailingThreadStatusStack.setContentCompressionResistancePriority(.required, for: .horizontal)
    ...
    private func rebuildTrailingThreadStatusSegments(summaries: [ThreadStatusSummaryDescriptor]) {
        let existing = trailingThreadStatusStack.arrangedSubviews.compactMap { $0 as? StatusSummaryButton }
        let existingKinds: [ThreadStatusSummaryKind] = existing.compactMap { button in
            guard let rawValue = button.identifier?.rawValue else { return nil }
            return ThreadStatusSummaryKind(rawValue: rawValue)
        }
        let summaryKinds = summaries.map(\.kind)
        guard existingKinds != summaryKinds else {
            refreshStatusButtonCountsInPlace(summaries: summaries)
            return
        }

        trailingThreadStatusStack.arrangedSubviews.forEach { subview in
            trailingThreadStatusStack.removeArrangedSubview(subview)
            subview.removeFromSuperview()
        }
        ...
        for summary in summaries {
            let button = makeStatusButton(for: summary)
            statusButtonsByKind[summary.kind] = button
            trailingThreadStatusStack.addArrangedSubview(button)
        }
    ...
    private func rebuildInlineThreadStatusBadges(groups: [InlineThreadStatusBadgeGroup]) {
        ...
        for group in groups {
            ...
            let groupGlyph = makeInlineStatusGroupGlyphButton(for: group.kind)
            threadStatusStack.addArrangedSubview(groupGlyph)
            threadStatusStack.setCustomSpacing(6, after: groupGlyph)
    ...
    private func inlineBadgeGroups(
        from threads: [MagentThread],
        settings: AppSettings,
        priority: [ThreadStatusSummaryKind]
        ...
        return priority.compactMap { kind in
            ...
            let badges = entries.map { entry in
                inlineBadgeDescriptor(for: entry.thread, kind: kind, settings: settings)
            }
    ...
    private func inlineBadgeWidth(for descriptor: InlineThreadStatusBadgeDescriptor) -> CGFloat {
        let titleFont = NSFont.systemFont(ofSize: 10.5, weight: .regular)
        let textWidth = ceil((descriptor.title as NSString).size(withAttributes: [.font: titleFont]).width)
        let repositoryWidth: CGFloat
        if let repositoryName = descriptor.repositoryName {
            let repositoryFont = NSFont.systemFont(ofSize: 8, weight: .regular)
            repositoryWidth = 8 + ceil((repositoryName as NSString).size(withAttributes: [.font: repositoryFont]).width)
        } else {
            repositoryWidth = 0
        }
        let horizontalPadding: CGFloat = 16
        return min(Self.maximumInlineBadgeWidth, max(Self.minimumInlineBadgeWidth, textWidth + repositoryWidth + horizontalPadding))
    }
    ...
    private func inlineBadgeDescriptor(
        ...
        let resolvedBranchName = branchName.isEmpty ? thread.name : branchName
        let worktreeName = (thread.worktreePath as NSString).lastPathComponent
        let projectName = settings.projects.first(where: { $0.id == thread.projectId })?.name
        let displayedProjectName = settings.projects.count > 1 ? projectName : nil
        ...
        return InlineThreadStatusBadgeDescriptor(
            ...
            threadId: thread.id,
            title: title,
            repositoryName: displayedProjectName,
    ...
    private static func truncatedInlineBadgeTitle(for thread: MagentThread) -> String {
        ...
        let end = title.index(title.startIndex, offsetBy: 58)
        return "\(title[..<end])..."
    }
    ...
    private func makeInlineStatusGroupGlyphButton(for kind: ThreadStatusSummaryKind) -> StatusSummaryButton {
        ...
        button.badgeTintColor = inlineBadgeTintColor(for: kind)
        button.contentTintColor = inlineBadgeTintColor(for: kind)
        button.target = self
        button.action = #selector(threadStatusTapped(_:))
        button.bezelStyle = .inline
        button.focusRingType = .none
        button.setButtonType(.momentaryChange)
        button.toolTip = kind.popoverTitle
        button.setAccessibilityLabel(kind.popoverTitle)
    ...
    @objc private func markDoneThreadAsReadFromMenu(_ sender: NSMenuItem) {
