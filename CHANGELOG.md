...
### Terminal
- Fixed significant UI lag introduced with link hover detection: the Ghostty word-under-cursor lookup (C API) and NSDataDetector URL check were running synchronously on the main thread for every mouse-move event; both are now debounced behind the existing 45 ms probe so they never block the main thread. The probe also now skips all work (C API, tmux) when Ghostty already owns the hover state via OSC 8 link detection.
