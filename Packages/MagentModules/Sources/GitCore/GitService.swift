...
public final class GitService: Sendable {
    ...
    public func fetchPullRequest(remote: GitRemote, branch: String) async -> PullRequestInfo? {
        if case .found(let info) = await lookupPullRequest(remote: remote, branch: branch) {
            return info
        }
        return nil
    }

    public func lookupPullRequest(remote: GitRemote, branch: String) async -> PullRequestLookupResult {
    ...
    private func fetchGitHubPR(remote: GitRemote, branch: String) async -> PullRequestLookupResult {
        if _ghAvailable == nil {
    ...
    private func fetchGitLabMR(remote: GitRemote, branch: String) async -> PullRequestLookupResult {
