...
public enum IPCAgentDocs {
    ...
    Use archive-thread --skip-local-sync to avoid writing local sync path changes into the main worktree during archive. archive-thread is refused when the worktree is dirty (uncommitted/untracked changes) OR when notable ignored files would be deleted. With dirty worktrees, --force auto-commits tracked/untracked changes with a generic message (`Uncommitted changes on <branch> (<worktree>)`) before archiving. Ignored files are still deleted by archive, so back them up first when needed. Prefer manual commit first for a more meaningful message.
    ...
    Archive safety: archive-thread is REFUSED when the worktree is dirty (uncommitted/untracked changes) OR when clean-but-notable ignored files would be deleted, unless --force is passed. The refusal message names the worktree path and explains the consequences. For dirty worktrees, --force auto-commits tracked/untracked changes with a generic message before archive. Ignored files remain destructive and are deleted with the worktree directory. Prefer manual commit first (better message), and back up ignored files before forcing archive.
    """
