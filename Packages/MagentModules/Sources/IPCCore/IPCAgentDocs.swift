...
public enum IPCAgentDocs {
    ...
    Use archive-thread --skip-local-sync to avoid writing local sync path changes into the main worktree during archive. archive-thread is refused when the worktree is dirty (uncommitted/untracked changes); --force bypasses this guard but is DESTRUCTIVE (it runs `git worktree remove --force`, which deletes the worktree dir and abandons uncommitted/untracked work). Do not pass --force unless the user has explicitly confirmed they want to discard the flagged data.
    ...
    Destructive-archive safety: archive-thread is REFUSED when the worktree is dirty (uncommitted/untracked changes), unless --force is passed. The refusal message names the worktree path and explains the consequences. Do NOT reflexively retry with --force — archive runs `git worktree remove --force`, which deletes the worktree directory and abandons uncommitted/untracked work. Only pass --force after the user has explicitly confirmed they want to discard the flagged data in this worktree. If unsure, commit/stash first.
    """
