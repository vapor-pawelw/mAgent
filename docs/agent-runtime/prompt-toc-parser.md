...
- The visible agent tab is rescanned every three seconds and shortly after Escape steering. Persisted prompt history is the durable TOC cache: live tmux entries are merged into its tail, so history-limit eviction or an empty recreated pane cannot remove older rows.
- Cached prompts that no longer occur in the complete tmux scrollback remain available for copy and rename and do not offer jump navigation. Rows with persisted timing continue to show their immutable start time and worked duration; `Outside terminal history` is shown in the tooltip instead of replacing that timing. Untimed rows show `Outside terminal history` in the footer.
...
- The pin button appears in the top-right only while the floating TOC is expanded. Pinning moves the TOC into a full-height trailing split and reserves its width so terminal and chat content cannot render underneath it. Unpinning immediately restores the content area's full width and explicitly resynchronizes the active Ghostty surface after Auto Layout completes.
- The pinned TOC stays expanded and always shows its unpin button. Its rectangular header and full-height divider meet the panel edges without inset rounding. The header and divider share the subtle neutral fill used by idle thread capsules, while the divider keeps the standard horizontal resize cursor across a forgiving 8pt hit target; drag it to resize the split. The chosen width and pin state persist across launches.
...
- The prompt list starts below the header with a small top inset so the newest row does not touch the header band.
...
- Refreshing keeps the last known prompt count in the badge; the spinner communicates loading without replacing the count with an ellipsis.
...
- Completion timing is the fast provisional path. Once `sessionConversationIDs` identifies the agent transcript, the TOC reads Claude `user`/`result` or Codex `user_message`/`task_complete` timestamps off the main actor and reconciles them back into the persisted timings. Transcript values replace matching provisional records while unmatched locally observed prompts are preserved; completed transcripts are not rescanned until the cached prompt count grows.
- Timing records are matched back to TOC entries newest-first by normalized prompt text, so a newly observed repeat such as `continue` does not donate its timestamp to an identical historical row. Long locally typed prompts may be retained as a suffix by the terminal input observer, so sufficiently long suffix matches are accepted for timing; short fuzzy matches are deliberately rejected. Cache reconciliation itself remains exact and caps duplicate live renderings only when timing metadata provides a trustworthy occurrence count; without timings, live rows remain authoritative so a new repeated submission is not discarded.
...
Right-clicking a TOC entry shows `Rename thread…` and `Rename tab`. `Rename thread…` opens the prompt-based rename sheet (`AIRenameSheetController`) pre-filled with the selected prompt text. The sheet explicitly identifies the flow as `Rename Thread with Prompt`, provides a multi-line editor, a picker with the last 10 recent prompts, and checkboxes to selectively rename icon, description, and/or branch name. The Icon option is omitted when automatic work-type icons are disabled in Settings, and the corresponding AI request does not ask for a work-type classification. On submit, it calls `renameThreadFromPrompt` with the selected options — bypassing all first-prompt eligibility gates (`didAutoRenameFromFirstPrompt`, `isAutoGenerated`, agent-process-detection) so it can be triggered at any time.

`Rename tab` immediately generates a concise tab label from the selected prompt for the active terminal tab. It does not open the thread rename sheet, does not rename the tmux session, and intentionally replaces either a default or manually assigned display label. The tab shows the same prompt-based naming activity feedback used by automatic tab naming.
...
**Background generation isolation:** Every thread/tab AI rename, slug, task-description, and icon generation path uses the same isolated CLI command builders. Claude runs with `--tools ""`, `--setting-sources ""`, and `< /dev/null`; Codex runs with `--ignore-user-config`, `--ignore-rules`, `--ephemeral`, and `< /dev/null`. This prevents either agent from loading MCP servers, tools, or repository instructions for these small text-only requests while retaining normal CLI authentication. Without that isolation, an unrelated unavailable MCP server or large project instruction set can exhaust the per-agent timeout and make naming fail silently.
...
**Provisional creation descriptions:** New threads immediately display a normalized preview of their initial prompt, or `Thread #N` when no prompt exists. These values set `taskDescriptionIsProvisional`; they are deliberately treated like an absent description by combined branch/description auto-rename and the independent task-description generator. A failed initial generation therefore leaves a useful title without preventing a later automatic generation from replacing it. Explicit user descriptions and Jira-synced descriptions are non-provisional and retain the existing overwrite protection.
