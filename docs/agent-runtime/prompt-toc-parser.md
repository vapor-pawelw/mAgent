...
Right-clicking a TOC entry shows a context menu with "AI Rename…". This opens the AI Rename sheet (`AIRenameSheetController`) pre-filled with the selected prompt text. The sheet provides a multi-line editor, a picker with the last 10 recent prompts, and checkboxes to selectively rename icon, description, and/or branch name. The Icon option is omitted when automatic work-type icons are disabled in Settings, and the corresponding AI request does not ask for a work-type classification. On submit, it calls `renameThreadFromPrompt` with the selected options — bypassing all first-prompt eligibility gates (`didAutoRenameFromFirstPrompt`, `isAutoGenerated`, agent-process-detection) so it can be triggered at any time.
