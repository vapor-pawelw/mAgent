...
- **Exited-agent recovery**: the same process scan distinguishes a truly idle shell from a tab running another command. When an agent tab's pane command is a supported shell and that shell has no child processes, the visible tab offers a **Start Agent** banner action that injects the managed `start-agent` helper. The offer is hidden while any foreground or background child process is present.
  - Require both persisted agent metadata and a non-terminal tab name. Legacy session migration can leave terminal-only sessions in the agent-session list, so `Terminal` / allocator-suffixed terminal tabs must never receive the offer.
  - Delay the offer for 7 seconds and recheck eligibility before revealing it. A bounded 5-second recheck window tolerates a transient unknown process snapshot without leaving a task alive indefinitely. This spans normal 5-second monitor ticks and prevents the banner from flashing while an agent process is starting.
  - Treat a failed `ps` snapshot as unknown, never as an empty child-process list. The banner must not appear unless Magent successfully confirmed that the shell has no children.
  - After **Start Agent** is accepted, suppress duplicate offers while startup begins. If the pane is still at the shell after 15 seconds, allow the action again so a failed launch does not permanently remove recovery.
  - Pending-prompt, injection-failure, and recovered-prompt banners take priority and must evict the idle-shell banner rather than overlap it.
  - `shellOnlyAgentSessions` is transient, lock-protected process state. Re-key it with other per-session state when tmux sessions are renamed and notify terminal views after changes.
- **Evidence-gated configured-type fallback**: when runtime detection is temporarily nil (for example while Codex is running `xcodebuild` in the pane), Magent may use stored `sessionAgentTypes` only as a weak hint. The hint is accepted only if recent pane content shows that agent's active busy marker (`esc to interrupt`/Codex Working markers), not just an idle prompt glyph (`❯`/`›`). This prevents shell sessions (or scrollback) from being misclassified as active agent tabs.
