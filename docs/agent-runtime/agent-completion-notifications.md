...
- **Codex**: Magent adds `UserPromptSubmit` and `Stop` hooks to its private managed `CODEX_HOME/config.toml`. The prompt hook stores `active` in the pane-local `@magent_codex_turn_state` tmux option. The Stop hook changes it to `idle` and appends the session name and event timestamp to the same completion log used by Claude. User-configured hooks remain present because Magent appends its hook tables to the copied config rather than replacing them.
