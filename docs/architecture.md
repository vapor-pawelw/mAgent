...
- Row-height measurement for multiline thread descriptions must not depend on transient thread state that can change as a side effect of selection (for example `hasUnreadAgentCompletion`). If the rendered font can vary, measure with a stable worst-case font so selection does not make the row jump between one-line and two-line heights.
