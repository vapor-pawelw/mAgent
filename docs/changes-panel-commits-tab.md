...
- **Diff WebView readiness and rendered callbacks are independent failure points**: `InlineDiffViewController` queues renderer calls until the bundled page is ready, probes `window.magentDiffRenderer` on navigation finish as a fallback if the `ready` script message is missed, and reveals the WebView after successful `setDiff` evaluation even if the `rendered` script message is not delivered. Keep all three paths; otherwise the fixed Diff tab can spin forever despite the DOM having rendered correctly.
