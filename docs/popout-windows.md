...
- The main thread detail top bar also includes a bounded `CurrentThreadStripView` before the tab/action controls. It uses sidebar-style thread metadata (icon, primary description/branch, branch/worktree metadata, dirty dot) but stays single-line and capped in width so the tab scroller and existing action buttons retain space. Pop-out windows still rely on their separate `PopoutInfoStripView` chrome above the detail view.
