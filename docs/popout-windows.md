...
- Main-window thread controls live in the native window toolbar; pop-out thread windows keep their in-content strip and tab/action bar for now.
...
- The capsule action row is owned by the host: in pop-out windows `ThreadPopoutWindowController` installs PR/Jira into its own `infoStrip`; in the main window those same buttons are reparented into the toolbar cluster beside the current-thread tile.
- The main window toolbar includes a bounded `CurrentThreadStripView` before the tab/action controls, then a flexible spacer, then Settings/Recently Archived. The tile uses sidebar-style thread metadata (icon, primary description/branch, branch/worktree metadata, dirty dot) and sidebar-style state border colors, while the main detail content starts directly with the terminal surface.
...
- The main-window toolbar cluster includes the pop-out button next to Archive.
- That pop-out button is hidden for the main thread, hidden in pop-out windows themselves, and hidden when the thread is already popped out.
