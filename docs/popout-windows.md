...
- The main window toolbar includes a bounded `CurrentThreadStripView` before PR/Jira actions, the flexible spacer, and Settings/Recently Archived. It uses sidebar-style thread metadata (icon, primary description/branch, branch/worktree metadata, dirty dot) and sidebar-style state border colors, but stays single-line and capped in width so the native toolbar actions retain space. Pop-out windows still rely on their separate `PopoutInfoStripView` chrome above the detail view.
