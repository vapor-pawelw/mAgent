...
- Two post-build script phases run on every build:
  - `scripts/embed-changelog.sh` — copies root `CHANGELOG.md` into the app bundle's Resources, writes the short git commit hash to `BUILD_COMMIT`, and sets `CFBundleVersion` to the git commit count (incremental build number). Root `CHANGELOG.md` contains only the latest released version; the app reads this bundled changelog for the "Changelog…" menu item and the launch-time "What's New" window.
  - `scripts/sync-version-from-tag.sh` — patches `CFBundleShortVersionString` to match the latest git tag (e.g. `v1.5.4` → `1.5.4`) so debug builds show the real release version instead of the placeholder `1.0` baked into `Project.swift`. The script patches **both** the Tuist source plist at `${SRCROOT}/Derived/InfoPlists/Magent-Info.plist` (which Xcode's `Process Info.plist` phase reads from — script phases run before `Process Info.plist`, so patching only the built bundle would be clobbered) and the built bundle plist as defense-in-depth. The CI release workflow performs an equivalent `sed` patch on `Project.swift` before building, so this script is a no-op there.
...
- The bootstrap script now fills in `SDKROOT` from `xcrun` when needed, which keeps GhosttyKit builds working in CI and in shells that do not inherit Xcode's developer environment.
