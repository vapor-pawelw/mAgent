...
- Track real divider interaction across the complete mouse event stream, not opportunistically inside `splitViewDidResizeSubviews`. Resize callbacks can miss mouse-down or mouse-up, leaving stale drag state that misclassifies a later selection-driven layout pass.
- Before trusting tracked drag state, reconcile it with `NSEvent.pressedMouseButtons`; releasing outside the app must self-heal the state even if the local monitor missed mouse-up.
