...
- Review launches should override built-in agents to the flagship review models (`opus` for Claude, `gpt-5.5` for Codex) and pass elevated reasoning on startup (`high` by default; Option-held review menu uses `max` for Claude and `xhigh` for Codex).
