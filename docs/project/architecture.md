...
- **Visible detail views update renamed sessions in place**: the thread-change observer distinguishes a manual session re-key from add/remove/reorder/pin changes and routes it through `handleRename(_:)`. Do not rebuild with `setupTabs()` for a rename because the existing Ghostty surface remains attached across `tmux rename-session`.

Automatic tab naming is a display-name-only path. When a session's submitted-prompt history first transitions from empty to non-empty, `AgentSetupService` asks `RenameService` to generate a specific 1-3 word label from the first prompt. `AppSettings.autoRenameTabs` defaults to enabled and is checked before invoking an agent and before applying the result. Generation uses the same preferred-agent/fallback ordering as thread rename. It never renames the tmux session. A missing label or a stored label matching the session agent's canonical default shape remains eligible, including allocator-added uniqueness suffixes such as `Codex-1` and `Codex (5.6-sol, M)-2`; other custom labels and all sessions in `manuallyRenamedTabs` are preserved. The guard is repeated after the asynchronous model call so manual renames always win races. Successful prompt-based names join that protection set so model/effort detection cannot replace them. While generation is running, only the affected tab title pulses in the app primary color; `RenameService.autoTabRenameInProgress` is transient and publishes start/finish notifications so main and pop-out thread views stay in sync.

### 4.2 Prompt-Based Rename Reuse

The "AI Rename…" sheet (⌘⇧R, also in thread context menu and TOC right-click) reuses the same model payload path as first-prompt auto-rename so branch slug generation, task description generation, and icon suggestion stay behaviorally aligned. The sheet provides a multi-line prompt input, a picker with the last 10 recent prompts, and checkboxes to selectively rename icon, description, and/or branch name (state persisted in `AppSettings.aiRenameIcon/aiRenameDescription/aiRenameBranch`). When `autoSetThreadIconFromWorkType` is disabled, the Icon checkbox is hidden and background rename/description prompts omit work-type classification entirely.
...
The 2-8 word limit is a preference, not a hard cap: longer descriptions are persisted. In CLI/IPC agent-driven rename flows, Magent returns a warning so the agent can shorten them when needed.
...
All of those background generation paths, plus automatic tab naming and standalone task-description/icon generation, use centralized isolated CLI command builders. Codex ignores user configuration and project rules; Claude disables tools and setting sources. Rename helpers must not invoke either CLI directly, because loading an unrelated MCP server can consume the generation timeout before a name is returned.
...
**Thread-removal race:** AI generation can outlive the thread that requested it. Automatic and manual prompt-based rename paths must stop quietly when delete, archive, failed pending-thread creation, or project removal makes the thread inactive while generation is in flight. Rename operations carry a per-thread lifecycle generation: cleanup invalidates the generation before delete starts destructive work, and archive/restore cannot reactivate an older task even though the restored thread keeps its UUID. Error presentation also requires both the thread and its project to remain active. Branch rename, delete, and archive share one serial mutation gate; AI generation stays outside it, while the Git/persistence phase cannot overlap a destructive lifecycle operation.
...
- App primary color is resolved centrally by `AppTheme` from `AppSettings.appPrimaryColorHex`. Primary actions, selections, highlights, and default user chat bubbles use `NSColor.appPrimary`; semantic status/provider colors remain independent. Update `AppTheme` before broadcasting settings changes so open-window refreshes resolve the new color.
...
- Dirty worktrees are always refused by default, including CLI `--force`. GUI archive can continue only through an explicit `Commit & Archive` flow where the user confirms and edits the commit message before archive. Ignored files remain destructive because they are not tracked by git.
...
- The main window uses `.fullSizeContentView`, and the sidebar split item opts into `allowsFullHeightLayout`. The sidebar scroll view spans the raw leading edge, while a safe-area-sized top content inset keeps resting rows clear of the traffic-light controls and allows scrolled rows to pass beneath them. The sticky-header overlay and empty-state content start at the safe-area top. While a sticky header is active, one hit-test-transparent blur surface extends from the raw window top through the header; hide it entirely with the sticky header to preserve the idle sidebar appearance. The main detail container's child also starts at its safe-area top so the existing tab bar remains below the window toolbar. Preserve the window's existing toolbar style, and keep the tracked sidebar separator immediately before the current-thread toolbar item so only that top strip moves to the detail side of the divider.
...
- Own real divider interaction in `SidebarTrackingSplitView.mouseDown(with:)`: set drag state immediately before calling `super`, then clear it with `defer` when AppKit's synchronous tracking loop returns. This uses AppKit's own effective divider hit target and avoids timing gaps from external event monitors or `NSEvent.pressedMouseButtons` polling.
- Keep Magent's divider-position policy self-contained in `splitView(_:constrainSplitPosition:ofSubviewAt:)`. The controller is already the split view's delegate, so forwarding the constraint callback to the superclass can re-enter AppKit's controller policy while Magent is enforcing a saved width.
- Outside a real divider drag, sidebar collapse animation, or explicit content-swap enforcement, `splitView(_:constrainSplitPosition:ofSubviewAt:)` must return `preferredSidebarWidth`. Delayed child constraint resolution can otherwise move the divider after the content-swap restoration callbacks have finished.
...
- Launch-time "center selected thread" behavior must be quiescence-aware: do not mark centering complete before a selected thread exists, debounce/cancel pending centering while startup reloads are still arriving, and verify the selected row is actually visible after animation before setting the one-shot "did center" guard. Otherwise late `reloadData`/scroll-restore passes can override centering and leave the selected row off-screen.
- Sidebar hide/show (via `View > Hide Sidebar`, the `Toggle Sidebar` keybinding, or `NSSplitViewController.toggleSidebar:`) is persisted to `UserDefaults` under `MagentSidebarHidden` and seeded into `sidebarItem.isCollapsed` directly (not via the animator) before the split view item is added, so the initial collapsed state paints without a launch-time animation. The width-restoration path (`applyInitialSidebarWidthIfNeeded`, `setPosition:ofDividerAt:`) must skip when the sidebar is collapsed, and `splitViewDidResizeSubviews` must ignore intermediate frames during the collapse/expand animation (`isTogglingSidebarCollapse` flag set around `toggleSidebar`/`revealSidebarIfHidden`), otherwise the preferred-width snap-back fights every animation frame.
- Auto-reveal of a hidden sidebar is opt-in on the `.magentNavigateToThread` notification: posters that represent a user-driven focus change (status bar popovers, thread info top strip click, system notification taps, restore-archived flows) must set `userInfo["revealSidebarIfHidden"] == true`. Programmatic/background navigations must omit the flag so they don't silently un-hide. Pop-out close/return flows route through a separate path and never trigger the reveal.
...
- Jira branch-ticket detection can be narrowed with `AppSettings.jiraTicketDetectionPrefixes`, a comma/semicolon-separated prefix allowlist normalized case-insensitively (for example: `IP, APPL, UT`). The filter applies only to branch-derived ticket detection; explicit synced `jiraTicketKey` values bypass it. Prefix edits trigger a debounced local sidebar reload through `SidebarSettingsFingerprint`; they must not invoke Jira/acli verification. Future Jira UI/cache code should resolve branch-based tickets through `MagentThread.effectiveJiraTicketKey(settings:)` so the allowlist is respected consistently.
- Jira ticket details shown on thread rows are force-verified during the shared 5-minute status sync and when an agent completion is detected for that thread. Opportunistic detection refreshes still use the Jira ticket cache, but displayed-status sync and completion refreshes bypass it so status badges do not wait for the cache TTL.
...
Shell startup uses a managed `ZDOTDIR` wrapper with separate terminal and agent profiles, while ensuring both profiles land in the intended worktree/repo directory.
...
- Agent startup should flow through `agentStartCommand(...)`, which launches an isolated interactive login zsh profile. The agent profile must not source user dotfiles, themes, aliases, functions, or plugin managers. It supplies only a deterministic executable `PATH`, Magent's working-directory setup, and Magent helpers such as `start-agent`. The profile remains active if the agent exits back to a shell and is inherited by agent-spawned shells, preventing nested commands from reintroducing interactive startup hooks or plugin update checks. Initialize the managed path after macOS `path_helper`, put conventional user-local agent locations first, retain system-managed path entries, and apply it only once so nested shells preserve virtualenv and project-specific path changes. Tools outside the managed `PATH` require an absolute path or a conventional executable location added by Magent.
- Agent binary invocations must use the `command <agent>` shell built-in prefix (e.g. `command claude`, `command codex`). This is defense-in-depth against wrappers if a command is launched outside the isolated profile and makes binary-only resolution explicit.
- Managed shells expose `start-agent`, which resolves the current tmux session through `/tmp/magent-cli start-agent` and runs the session-specific launch/resume command in the current terminal. Recovery UI should inject `start-agent`, not the expanded command, so users see the same helper they can type manually after an agent exits.
- Codex sessions must run with a shared Magent-managed `CODEX_HOME` (under `/tmp`) instead of writing Magent IPC hints into the global `~/.codex/AGENTS.md`. The managed home mirrors most top-level entries from `~/.codex` via symlinks (skills/auth/state/etc.) but owns local `AGENTS.md` and `config.toml`. `AGENTS.md` is built as: user `~/.codex/AGENTS.md` content + Magent IPC block when prompt-injection is enabled. `config.toml` is copied from the user's global config and only applies compatibility migrations needed for Magent-launched Codex sessions, such as `features.codex_hooks` -> `features.hooks`; never inject Magent lifecycle hooks or mutate the user's global config. Non-managed Codex command hooks require explicit user trust, so Magent-owned runtime detection must remain app-side. A lightweight metadata sync (size/mtime/existence for `~/.codex/AGENTS.md`, `~/.codex/config.toml`, and `~/.codex`) refreshes the managed home on the session-monitor fast tick (~5 s) so global edits/additions/removals propagate without restarting Codex tabs.
- The Codex `env`-wrapper command must list option flags (e.g. `-u NO_COLOR`) **before** any `NAME=value` assignments. macOS BSD `env` stops option parsing at the first assignment and treats remaining args as the command, so `env CODEX_HOME=... -u NO_COLOR command codex ...` fails with `env: -u: No such file or directory`. Use `env -u NO_COLOR CODEX_HOME=... command codex ...` instead (see `AgentSetupService.codexSessionEnvironmentWrappedCommand`).
- Codex App project synchronization is additive and Git-aware. `CodexAppProjectSyncService` preserves unknown values and existing path spellings while reconciling the recognized project arrays and conversation-to-workspace hints in `~/.codex/.codex-global-state.json`; it refuses an unexpected value shape instead of overwriting a newer schema. Repository-identity migration runs after thread restoration even when Codex sync is disabled; Codex import, export, and conversation assignment require `syncCodexAppProjects` plus Codex being enabled or already used by a persisted session. The coordinator watches lightweight file metadata every 5 seconds so projects added in Codex App are discovered without relaunching Magent. Magent project roots are exported, Codex workspace roots that resolve to Git repositories are imported, and linked worktrees resolve through `git worktree list` to the main repository before de-duplication. Imported projects default to Codex. Never propagate project deletion between apps; a project explicitly removed from Magent is recorded in `codexAppProjectImportExclusions` so a root retained by Codex App does not immediately recreate it.
- A Codex conversation is assigned to `Project.repoPath`, not its thread's `worktreePath` or the configurable `worktreesBasePath`. This keeps every Magent worktree grouped under one Codex App project even when worktrees live on another disk or several projects share a worktree parent directory. Only sessions whose resolved `AgentType` is `.codex` contribute conversation hints; Claude and terminal tabs remain untouched.
- Project identity combines the resolved path returned by `git rev-parse --path-format=absolute --git-common-dir` with the configured path relative to its worktree root. The first launch reconciliation starts only after persisted threads finish restoring, then `MagentProjectDeduplicator` maps equivalent main-checkout, linked-worktree, and symlink paths onto that identity while keeping distinct monorepo subdirectories as separate projects. Existing duplicate Magent projects keep the first project in sidebar order, normalize its `repoPath` to the regular repository's primary worktree (plus the same relative subdirectory), merge additive project configuration, retarget active and archived threads, and run the standard duplicate-thread consolidation (using symlink-resolved worktree paths) before removing redundant project records. An intentionally registered lone linked worktree keeps its configured project path; canonicalization to the primary checkout happens only when aliases are actually merged or resolve to the same directory. Bare repositories have no stable primary checkout, so their first surviving registered worktree remains the canonical path and a duplicate project's distinct main checkout becomes a regular thread under it. A main thread whose path used an alias follows the canonical project path; other threads retain their explicit worktree paths even when they live outside the surviving project's `worktreesBasePath`. Retargeting uses a structural copy and consolidation must preserve all persisted tab state. Thread persistence is rolled back if the following settings write fails. Import candidates and removal exclusions use the same identity, so several Codex roots for one project can create at most one Magent project; if any registered project cannot be resolved, importing pauses for that pass rather than risking a duplicate.
- In standard terminal tabs, a one-time `cd` in user `.zshrc` is expected and should be overridden by `MAGENT_START_CWD` during session creation.
...
- Review launches should override built-in agents to the flagship review models (`fable` for Claude, `gpt-5.5` for Codex) and pass elevated reasoning on startup (`high` by default; Option-held review menu uses `max` for Claude and `xhigh` for Codex).
...
- Thread switching has two explicit fast paths for healthy sessions: `ThreadDetailViewController` can restore a cached `TerminalSurfaceView` wrapper for a session and pass that resource-backed state into `ThreadManager.isSessionPreparedFastPath(...)`, while `ThreadManager.recreateSessionIfNeeded(...)` keeps a short-lived "known good" session-context cache as a fallback for recently validated sessions without a retained surface.
- Fast-path eligibility must survive `ThreadDetailViewController` recreation: `ensureSessionPrepared(...)` should consult `ThreadManager.isSessionPreparedFastPath(...)` before issuing any tmux checks so quick A→B→A revisits do not re-run `tmux has-session` just because the new VC starts with an empty local `preparedSessions` set. Dead, evicted, unreferenced, and in-recreation sessions must still take the validation/recreation path.
- Restore the selected cached terminal surface before constructing secondary terminal views. Cache compatibility must use stable session ownership/context fields rather than the generated startup command; conversation/resume metadata can change while a healthy surface is detached. The `TmuxService` pre-kill hook invalidates the cache entry before freeing surfaces so same-name session recreation cannot reuse a stale view.
...
- Slow non-agent terminal-tab switches should still use that debounced overlay path while `ensureSessionPrepared(...)` validates tmux/session health. Do not short-circuit to "no overlay" just because no agent prompt injection is expected.
...
- Apply that retention rule only when the selected tab still resolves to a live agent session at runtime. If the pane resolves as a plain terminal (`agentType == nil`), clear any startup-overlay token instead of leaving `Preparing terminal session...` on top of an interactive shell/Codex view.
...
- **Two-phase completion must tolerate concurrent structure rebuilds**: while `addTab()` is awaiting tmux/session setup, `.magentThreadsDidChange` can trigger `setupTabs()` and replace the entire tab strip. On success, reconcile by session name (`alreadyPresent` / `replacePending` / `append`) instead of assuming the original pending index is still valid.
- **Local create flow owns selection during auto-switch creation**: while a local `addTab()`/draft-to-agent creation with "Switch to new tab" is still in flight, `handleThreadsDidChange` must suppress structure-driven `setupTabs()` rebuilds and let the local placeholder→session reconciler finish. Otherwise `setupTabs()` can briefly restore the previously selected tab and cause visible old/new tab flicker on fast `Cmd+T` submits.
- Terminal-view caching preserves the live Ghostty surface: `ReusableTerminalViewCache.store(...)` sets `preserveSurfaceOnDetach` before removing the view, and reattachment clears that flag while keeping the existing surface. Every cache eviction must therefore explicitly free the surface on the main actor, and every tmux kill path must invalidate matching cached entries before the PTY closes.
...
- **Terminal surface memory pressure**: the app monitors macOS dispatch memory-pressure events independently of tmux idle eviction. Warning pressure retains only the two newest detached cached surfaces; critical pressure frees every detached cached surface. Trimming is main-actor and explicitly destroys Ghostty surfaces, but it never kills tmux sessions or touches visible/pop-out surfaces.
...
- **Agent completion detection**: Magent no longer relies on per-session tmux `pipe-pane` bell watchers by default. Claude completion attention comes from the Magent-injected Claude Stop hook; Codex completion attention is synthesized from the session monitor when a Codex session transitions from busy to idle. Codex pane parsing resolves the newest meaningful state, so a newer `›` prompt supersedes stale `Working`/background-terminal text without requiring Codex hooks. `TmuxService.legacyAgentBellPipeEnabled` preserves the old pipe-pane path as an emergency rollback switch, and `ensureBellPipes()` detaches legacy pipes from upgraded live sessions while that flag is off.
...
- `.draft(identifier:)` — content is in `draftTabs`, keyed by identifier; persisted in `thread.persistedDraftTabs` including agent type, prompt, optional model/reasoning overrides used by later `Start Agent`, and `isPinned`
- `.chat(identifier:)` — content is in `chatTabs`, keyed by identifier; persisted in `thread.persistedChatTabs` including full message history, per-tab unsent composer draft text, draft attachments, sent message attachments, and `isPinned`
...
- `webTabs`, `draftTabs`, and `chatTabs` are restored in persisted order, with pinned entries inserted into the unified pinned region
- Draft/chat view controllers are created lazily on first selection
- `tabSlots` + `tabItems` change order during drag/pin operations; `persistTabOrder()` syncs `terminalViews`, `thread.tmuxSessionNames`, non-terminal tab order, and per-tab pin state to match
...
- Single unified `pinnedCount` covers all movable tab types. The fixed Terminal and Diff tabs are never pinnable.
...
- External tab-structure mutations (for example CLI `create-tab` or `pin-tab`) must be reconciled in existing thread views. `ThreadDetailViewController.handleThreadsDidChange` compares a tab-structure fingerprint (terminal sessions + pinning + persisted web/draft/chat identifiers and pin state) and reruns `setupTabs()` when structure changes are detected.
...
**Serialized archive cleanup pattern**: `ThreadLifecycleService.archiveThread` runs through one `SerialAsyncOperationGate`. It removes the thread from UI state first, then awaits `@concurrent` cleanup (deferred Local Sync, tmux session kills, worktree removal, symlink sweeps, stale-session pruning) before allowing the next archive request to proceed. This keeps the UI responsive without letting multiple archive tasks race on shared persistence, sync destinations, or git worktree metadata. Each queued request re-resolves its thread by ID after acquiring the gate because earlier archive work may have removed it while the request was suspended. Tmux sessions belonging to the one active archive may still be killed concurrently.
