...
- The main window uses `.fullSizeContentView`, and the sidebar split item opts into `allowsFullHeightLayout`, while its list, sticky header, and empty-state content start at the sidebar view's safe-area top. This lets the sidebar background fill the entire leading edge behind the traffic-light controls without placing rows underneath them. The main detail container's child also starts at its safe-area top so the existing tab bar remains below the window toolbar. Preserve the window's existing toolbar style, and keep the tracked sidebar separator immediately before the current-thread toolbar item so only that top strip moves to the detail side of the divider.
