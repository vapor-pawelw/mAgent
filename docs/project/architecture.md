...
- The sidebar split item opts into `allowsFullHeightLayout`, while its list, sticky header, and empty-state content start at the sidebar view's safe-area top so the background reaches behind the traffic-light controls without placing rows underneath them. Do not opt the whole window into `.fullSizeContentView` or force a unified toolbar: that changes the detail area's vertical chrome and can move the existing tab bar beneath the top toolbar. Keep the tracked sidebar separator immediately before the current-thread toolbar item so only that top strip moves to the detail side of the divider.
