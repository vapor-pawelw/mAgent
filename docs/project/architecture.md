...
- Codex sessions must run with a shared Magent-managed `CODEX_HOME` (under `/tmp`) instead of writing Magent IPC hints into the global `~/.codex/AGENTS.md`. The managed home mirrors top-level entries from `~/.codex` via symlinks (skills/auth/config/state/etc.) but owns its local `AGENTS.md`, built as: user `~/.codex/AGENTS.md` content + Magent IPC block when prompt-injection is enabled. The global file remains user-controlled. A lightweight metadata sync (size/mtime/existence for `~/.codex/AGENTS.md` and `~/.codex`) refreshes the managed home on the session-monitor fast tick (~5 s) so global edits/additions/removals propagate without restarting Codex tabs.
