...
- Codex sessions must run with a shared Magent-managed `CODEX_HOME` (under `/tmp`) instead of writing Magent IPC hints into the global `~/.codex/AGENTS.md`. The managed home mirrors most top-level entries from `~/.codex` via symlinks (skills/auth/state/etc.) but owns local `AGENTS.md` and `config.toml`. `AGENTS.md` is built as: user `~/.codex/AGENTS.md` content + Magent IPC block when prompt-injection is enabled. `config.toml` is copied from the user's global config, applies compatibility migrations needed for Magent-launched Codex sessions such as `features.codex_hooks` -> `features.hooks`, and appends Magent's `UserPromptSubmit`/`Stop` lifecycle hooks while preserving user hook tables. The hooks store pane-local active/idle state and route Stop events through the shared completion log; they must remain scoped to the managed copy and never mutate the user's global config. A lightweight metadata sync (size/mtime/existence for `~/.codex/AGENTS.md`, `~/.codex/config.toml`, and `~/.codex`) refreshes the managed home on the session-monitor fast tick (~5 s) so global edits/additions/removals propagate without restarting Codex tabs.
...
- **Agent completion detection**: Magent no longer relies on per-session tmux `pipe-pane` bell watchers by default. Claude and Magent-launched Codex sessions route Stop-hook completion events through the shared event log. Codex additionally stores active/idle state in a pane-local tmux option; confirmed idle state overrides stale detached-pane status text only when the `›` prompt is also visible, so a missing submit hook cannot permanently hide a genuinely active turn. The session monitor's busy→idle transition remains the fallback when hooks are unavailable. `TmuxService.legacyAgentBellPipeEnabled` preserves the old pipe-pane path as an emergency rollback switch, and `ensureBellPipes()` detaches legacy pipes from upgraded live sessions while that flag is off.
