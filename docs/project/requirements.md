...
- Context menu order for non-main threads starts with `Pin/Unpin`, then `Add to Favorites` / `Remove from Favorites`, then `Fork Thread`, then Jira, then `Sign` (top-level), `Priority` (top-level), `Section` (top-level when other visible sections exist), `Customize` submenu (`AI Rename…`, Icon, Description, Branch name), then `Hide/Unhide`, then `Session` submenu (`Keep Alive`, `Kill All Sessions` when live sessions exist), and ends with `Archive` / `Delete`.
