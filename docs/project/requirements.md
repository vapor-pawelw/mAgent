...
- `Cmd+Shift+T` restores the most recently closed tab for the current thread (terminal, web, or draft), with an in-memory per-thread history capped at 10 entries. This history is not persisted across app restarts.
- Double-clicking a terminal session tab opens the same rename prompt as the tab context-menu "Rename Tab..." action
- An idle Claude or Codex tab with a persisted conversation ID can move to a new thread. The destination must strictly resume the same conversation, preserve user-facing tab metadata, and receive staged, unstaged, and non-ignored untracked changes from the shared source worktree with rollback on failure. The source tab disappears only after destination readiness is confirmed; a failed resume must never degrade to a fresh conversation. Interrupted moves surface persistent recovery guidance; Retry is offered only for redundant-stash cleanup, never for conflicts or states that require user action.
...
- URL normalization (shared `WebURLNormalizer`): bare hostnames get `https://`, localhost/loopback addresses get `http://`, `host:port` patterns without `://` are detected and normalized; host:port entries with paths/query/fragment (for example `localhost:3000/docs?q=1#api`) must preserve those components
- Only `http` / `https` URLs are eligible for in-app web tabs and persisted web-tab current URLs. Non-web schemes (for example `mailto:`, app-deep links, or custom handlers) must open in the default browser/OS handler instead of loading in the embedded WKWebView.
...
- **Option+middle-click** on any tab closes it immediately without a confirmation alert
...
- Context menu order for non-main threads starts with `Pin/Unpin`, then `Add to Favorites` / `Remove from Favorites`, then `New Thread from This Thread…`, then Jira, then `Sign` (top-level), `Priority` (top-level), `Section` (top-level when other visible sections exist), `Customize` submenu (`AI Rename…`, Icon, Description, Branch name), then `Hide/Unhide`, then `Session` submenu (`Keep Alive`, `Kill All Sessions` when live sessions exist), and ends with `Archive` / `Delete`.
- Priority submenus opened from either the thread context menu or the sidebar priority indicator append `(Jira)` to the level matching the verified Jira ticket priority.
...
- If `taskDescription` exists:
  - Line 1: description (1 line by default, or up to 2 lines when `Wide threads` is enabled)
