...
> **Agent selection**: `--agent` determines the agent type and surface for the thread's *initial tab*. Use `--agent codex:chat` for a Codex chat-first thread. Claude currently supports terminal tabs only. Omitting `--agent` creates the initial tab using the project/global default agent.
...
> **Agent selection**: Set `agentType` per spec to control which agent and surface each thread opens with. Use `"agentType": "codex:chat"` for Codex chat-first threads. Omitting it falls back to the project/global default.
...
```bash
magent-cli create-tab --thread <name> [--agent claude|codex|custom|terminal] [--model <id>] [--reasoning none|low|medium|high|xhigh|max|ultra] [--name <text> | --title <text>] [--fresh|--no-resume] [--prompt <text>]
```
