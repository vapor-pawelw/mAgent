...
- `Threads` owns thread naming defaults, thread sections, recently archived thread restore history, startup injection fields, the review prompt, and sidebar display options (narrow threads, PR/Jira status badge toggles, busy/idle duration toggle).
