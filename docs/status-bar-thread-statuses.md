...
- Inline thread badges show the thread description when present, otherwise the thread name, capped at 40 characters. When multiple projects are configured, the badge text includes the project name as a dimmed prefix.
- Inline badges are grouped by logical status with one tinted status glyph before each group and subtle vertical separators between groups. Badges do not repeat the glyph or show status words; the shared glyph color, matching border color, and tooltip provide the context. Rate-limited threads do not expand inline; when any rate-limited status is active, the left side stays in aggregate mode so the existing rate-limit count and popover remain visible.
