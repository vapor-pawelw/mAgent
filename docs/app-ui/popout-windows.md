...
- Main-window thread identity now lives in the native window toolbar instead of an in-content thread strip. Pop-out thread windows keep their in-content strip for now.
...
- The capsule action row is owned by the host: in the main window the `ThreadDetailViewController` exposes PR/Jira buttons to the window toolbar next to `CurrentThreadStripView`; in pop-out windows `ThreadPopoutWindowController` installs the same buttons into its own `infoStrip`. The detail VC styles them as inline capsules in either case (`.inline` bezel, `.small` control size), and skips adding them to its tab/action bar so they don't appear twice.
- The main window toolbar includes a bounded `CurrentThreadStripView` before PR/Jira actions, the flexible spacer, and Settings/Recently Archived. It uses sidebar-style thread metadata (icon, primary description/branch, branch/worktree metadata, dirty dot) inside a single passive toolbar capsule, and the thread text can expand across available titlebar space before truncating. Pop-out windows still rely on their separate `PopoutInfoStripView` chrome above the detail view.
- When toolbar width is constrained, the thread summary yields its preferred width before the PR/Jira capsules compress. Titled action capsules remain single-line so labels such as `Create PR` do not wrap into unreadable fragments.
...
- The main-window tab/action bar (above the terminal) still includes a pop-out button next to Archive.
...
- Pop-out windows host their own `BannerOverlayView` registered with `BannerManager.shared.registerContainer(_:)` at setup and unregistered in `tearDown()`. Without this, errors surfaced during work triggered from a pop-out (AI rename failures, etc.) would only render in the main window and be invisible when the user has the main window hidden or behind the pop-out. See `docs/app-ui/banner-behavior.md` → "Multi-window routing".
- Thread archive/delete must tear down pop-out windows synchronously before any `tmux kill-session` call. Both paths call `ThreadManager.releaseLivingGhosttySurfaces(for:)`, which invokes `PopoutWindowManager.returnThreadToMain(_:)` plus per-session `returnTabToThread(sessionName:)` and then evicts `ReusableTerminalViewCache` again (the return path re-caches the view with `preserveSurfaceOnDetach = true`). Archive may finish cleanup in the background; Delete waits for the worktree and branch to be removed before removing the thread from persistence/UI. Both paths also post `.magentArchivedThreadsDidChange` — `PopoutWindowManager.startObserving` listens for it as a belt-and-suspenders cleanup signal. See `docs/agent-runtime/libghostty-integration.md` → "Surface Lifecycle: Thread Archive/Delete Contract" for the full rationale.
