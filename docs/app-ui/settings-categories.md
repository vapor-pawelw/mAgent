...
- `AppSettings.syncCodexAppProjects` gates additive project synchronization. Turning it off stops both project import/export and Codex conversation assignment; turning it back on schedules an immediate reconciliation. Removing a project from Magent adds its repository path to `codexAppProjectImportExclusions`, preventing automatic re-import while leaving the Codex App project untouched.
