...
- `Agents` owns automatic Codex App project synchronization. It is enabled by default, becomes active when Codex is an enabled agent or an existing Magent tab has a Codex conversation, and never changes Claude sessions.
...
- `AppSettings.syncCodexAppProjects` gates additive project synchronization. Turning it off stops project import/export and Codex conversation assignment; repository-identity migration still consolidates duplicate Magent projects safely. Turning sync back on schedules an immediate reconciliation. Removing a project from Magent adds its repository path to `codexAppProjectImportExclusions`, preventing automatic re-import while leaving the Codex App project untouched.
...
- `PrimaryTintStyler` applies the resolved primary color to standard Settings selection controls, `PrimaryTintSlider` tracks, non-destructive rounded actions, launch-sheet primary actions, and the explicitly opted-in tab toolbar buttons. Sidebar thread/section drag-and-drop replaces AppKit's system-accent feedback with custom insertion and target indicators drawn by `SidebarOutlineView`. Keep destructive, branded-app, and semantic status actions out of the shared tint pass.
