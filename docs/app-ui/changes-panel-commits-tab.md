...
- **Right-clicking "Uncommitted"** opens a context menu with `Discard Changes`. Choosing it shows a destructive confirmation alert, then discards all uncommitted tracked/untracked paths in the thread worktree.
...
- `CommitRowView` — selectable NSView subclass (like `DiffFileRowView`) with `isSelected` highlight. Uses `"__uncommitted__"` as a sentinel hash for the Uncommitted row's `updateCommitRowSelectionAppearance`. Right-click on normal commit rows shows "Copy Hash" and "Copy Message"; right-click on the uncommitted sentinel row shows `Discard Changes`.
...
- **Agent completion is an immediate git-state refresh signal**: When any session in the active thread reports completion, run `ThreadManager.refreshGitStateAfterAgentCompletion()` to refresh dirty, delivered, branch, and model-derived tab state immediately, then reset the session monitor git-state cadence so the next periodic tick does not repeat the same work. The sidebar changes panel still refreshes via `refreshDiffPanel(..., resetPagination:false, preserveSelection:true)`. The fixed Diff tab must stay lazy: refresh the full diff content only when the inline viewer is already open; otherwise refresh only the tab title/count from branch-wide stats.
...
- **The fixed Diff tab count must use branch-wide stats, not `uncommittedEntries`**: the tab title represents the inline Diff viewer's default scope. For non-main threads, that is the full branch diff from merge-base to the working tree (committed + uncommitted changes), so a clean branch with committed changes must not show `Diff (0)`. Use `GitService.threadDiffTabStats(worktreePath:baseBranch:)` for title/count updates. `uncommittedEntries` remains working-tree-only and is only for the COMMITS tab's optional "Uncommitted" row.
- **The fixed Diff tab context label must match the loaded diff scope**: commit details show `Viewing commit: <title>`, forced working-tree mode shows `Uncommitted changes`, and normal non-main branch diffs show `All changes (<current branch> → <base branch>)`. If the resolved base branch is missing or matches the current branch, load and label the working-tree diff instead of presenting a branch range.
- **Diff WebView readiness, rendered callbacks, and renderer failures are independent failure points**: `InlineDiffViewController` queues renderer calls until the bundled page is ready, probes `window.magentDiffRenderer` on navigation finish and on a short readiness poll as a fallback if the `ready` script message is missed or the module becomes available after `didFinish`, reveals the WebView after successful `setDiff` evaluation even if the `rendered` script message is not delivered, and shows a native timeout message if the renderer never finishes. Timeout logging must include the compact render context plus a JavaScript page snapshot (document state, renderer presence, root/body size) so stuck cases are diagnosable from Console logs without dumping the full patch. The bundled renderer must also catch render exceptions, post `error` + `rendered` with source/stack/patch-size metadata, and replace the body with a failure row. Keep these paths; otherwise the fixed Diff tab can spin forever despite the DOM having rendered correctly or failed recoverably.
