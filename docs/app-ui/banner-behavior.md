...
- **Global-hosting gotcha**: app-wide top banners must be hosted on the window theme-frame parent above `contentView` when available, not inside `contentView` itself. In transparent-titlebar windows, the titlebar/theme frame can sit above `contentView` in the event path and make a visibly rendered banner untappable unless the overlay is mounted there. Keep the banner itself constrained to the overlay's safe-area top so it does not render under toolbar items such as the current-thread capsule.
