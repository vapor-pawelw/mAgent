...
- **Sticky headers**: When the user scrolls past a project or section header, a floating overlay (`StickyHeaderOverlayView`) pins the project name (and current section, if applicable) at the top of the sidebar. Sticky project headers keep the same trailing `+` add-thread button as the normal project row, including the project/agent menu behavior and disabled state while a thread is being created. A 12pt fade gradient softens the transition into scrolling content. Clicking the non-button area of a sticky header smoothly scrolls the sidebar to reveal the actual header row.
