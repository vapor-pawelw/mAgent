...
- For Homebrew installs, the flow is now two-phase to match the bundle-replacement path. Phase 1 (`runHomebrewPrefetch`) runs while the app is alive: `brew update --quiet` then `brew fetch --cask magent`, which drops the new cask artifact into the Homebrew cache while showing a "Downloading…" banner with a spinner. When that completes, the user sees the same persistent `Magent X.Y.Z is ready to install` banner used by the bundle path, with an `Install & Relaunch` button. Phase 2 (`performHomebrewRelaunch`) only runs when the user clicks that button: it writes the detached updater script (`writeHomebrewUpdaterScript`) and quits. The detached script waits for the process to exit, skips the second `brew update --quiet` when the in-app prefetch refreshed the tap within the last 10 minutes, runs `brew upgrade --cask magent` (fast because the artifact is already cached), clears launch-blocking xattrs, and reopens Magent. If Homebrew reports upgrade failure after the target version is already installed, the script skips the slow `brew reinstall --cask magent` fallback and continues to cleanup/relaunch.
- The Homebrew relaunch script writes its latest phase to `/tmp/magent-self-update.status` and emits macOS notifications for waiting, optional tap refresh, cask upgrade, cleanup, and relaunch. This gives users visible progress during the short window where Magent has already quit and cannot render banners.
...
- The Homebrew path MUST refresh the tap before `brew upgrade --cask magent` unless the same app session just did so during prefetch. The release workflow pushes the updated cask to `vapor-pawelw/homebrew-tap`, but a user's local tap clone is only refreshed on demand. Without a recent refresh, `brew upgrade --cask magent` compares against the stale cached cask version — which can be older than the currently installed app — and becomes a silent no-op. The app then relaunches at the same version, and the user sees "updating via Homebrew" followed by no visible change. The same reasoning applies to the in-app Phase 1 prefetch — `runHomebrewPrefetch` runs `brew update` before `brew fetch --cask magent` so the fetch targets the current cask revision, not a stale cache.
- The detached Homebrew script may skip its own `brew update` only when `runHomebrewPrefetch` refreshed the tap recently in the same app session. Keep that time-based guard if changing the relaunch flow; removing both refresh points can make updates silently no-op on stale taps, while always refreshing after quit lengthens the invisible restart gap.
- Keep the detached Homebrew reinstall fallback version-aware. `brew upgrade --cask magent` can return non-zero for benign already-current states; only run `brew reinstall --cask magent` when `brew list --cask --versions magent` does not match the target version.
