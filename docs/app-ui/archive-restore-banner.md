...
- **Clear sidebar selection for removed threads before reloading.** Archive/delete removes the thread from `ThreadManager.threads` before `ThreadListViewController` performs its structural reload. If the removed thread is still stored as `selectedThreadID` or `diffInspectionThreadID` during that reload, deferred sidebar geometry/diff refresh work can keep reading stale row state. `threadManager(_:didArchiveThread:)` and `threadManager(_:didDeleteThread:)` must call `clearRemovedThreadState(threadId:)` before `reloadData()`.
