...
- Permanent placement is identity-based, not index-based. `ThreadTabIdentity.permanentTerminal(sessionName:)` binds the fixed Terminal surface to its dedicated tmux session, while `.permanentDiff` identifies Diff. Agent, web, draft, and chat tabs remain movable even if persistence restores them before either permanent tab in the backing arrays.
...
- Never infer permanence from `tabItems[0]`, `tabItems[1]`, or a fixed-count prefix. Use the corresponding `ThreadTabIdentity`; persisted tmux-session order and visual tab order are intentionally independent.
